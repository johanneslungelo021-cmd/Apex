...
export function RichTextEditor({
  ...
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        codeBlock: {
          HTMLAttributes: {
            class:
              "bg-zinc-900 rounded-lg p-4 font-mono text-sm text-emerald-400 overflow-x-auto",
          },
        },
      }),
      TiptapImage.configure({
        ...
        allowBase64: false,
        HTMLAttributes: {
          class: "rounded-lg max-w-full h-auto my-4 border border-zinc-800",
        },
      }),
      Link.configure({
        openOnClick: false,
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-invert prose-zinc max-w-none focus:outline-none px-8 py-6 min-h-[500px] text-zinc-100 leading-relaxed",
      },
        ...
        if (!moved && event.dataTransfer?.files.length) {
          ...
          if (file.type.startsWith("image/") && onImageUpload) {
            ...
            onImageUpload(file)
              ...
              .catch(() => {
                /* silently ignore drag-drop upload errors */
              });
            return true;
          }
        }
  ...
  if (!editor)
    return <div className="h-96 bg-zinc-900 rounded-xl animate-pulse" />;
