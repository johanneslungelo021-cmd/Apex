...
function getScore(
  ...
  if (seoTitle && seoTitle.length >= 30 && seoTitle.length <= 60) score += 25;
    ...
    tips.push(
      seoTitle.length < 30
        ? "SEO title is too short (aim for 30–60 chars)"
        : "SEO title is too long (max 60 chars)",
    );
  if (seoDesc && seoDesc.length >= 120 && seoDesc.length <= 160) score += 25;
  else
    tips.push(
      seoDesc.length < 120
        ? "Meta description is too short (aim for 120–160 chars)"
  ...
  const generateSEO = async () => {
    ...
    try {
      ...
      if (titleSettled.status === "fulfilled" && titleSettled.value.ok) {
        ...
        if (titleData.result)
          updates.seo_title = titleData.result.substring(0, 60);
      }
      if (descSettled.status === "fulfilled" && descSettled.value.ok) {
        const descData = await descSettled.value.json();
        if (descData.result)
          updates.seo_description = descData.result.substring(0, 160);
      }
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span
            className={`text-xs ${seoTitle.length > 60 ? "text-red-400" : "text-zinc-500"}`}
          >
            {seoTitle.length}/60
          </span>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span
            className={`text-xs ${seoDescription.length > 160 ? "text-red-400" : "text-zinc-500"}`}
          >
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {metaKeywords.map((kw) => (
            <span
              key={kw}
              className="flex items-center gap-1 text-xs px-2.5 py-1 bg-blue-900/30 text-blue-300 border border-blue-800/50 rounded-full"
            >
              {kw}
              <button
                onClick={() =>
                  onChange({
                    meta_keywords: metaKeywords.filter((k) => k !== kw),
                  })
                }
              >
                <X className="h-3 w-3" />
