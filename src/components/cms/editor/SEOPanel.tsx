...
function getScore(seoTitle: string, seoDesc: string, content: string, keywords: string[]): { score: number; tips: string[] } {
  ...
  if (keywords.length >= 3) score += 25; else tips.push('Add at least 3 focus keywords');
  const wordCount = content.replace(/<[^>]*>/g,'').split(/\s+/).filter(Boolean).length;
  if (wordCount >= 300) score += 25; else tips.push(`Content too short (${wordCount} words — aim for 300+)`);
  return { score, tips };
}
  ...
  const generateSEO = async () => {
    ...
    try {
      const [titleRes, descRes] = await Promise.all([
        fetch('/api/cms/ai-generate', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      ...
      onChange({ seo_title: titleData.result?.substring(0, 60) ?? '', seo_description: descData.result?.substring(0, 160) ?? '' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4">
        ...
        {score === 100 && <p className="mt-2 flex items-center gap-1.5 text-xs text-emerald-400"><Check className="h-3.5 w-3.5" />All SEO checks passed!</p>}
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2 mb-2">
          ...
          <button onClick={addKeyword} className="px-3 py-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-sm text-white transition-colors">Add</button>
        </div>
        <div className="flex flex-wrap gap-1.5">
          {metaKeywords.map(kw => (
            <span key={kw} className="flex items-center gap-1 text-xs px-2.5 py-1 bg-blue-900/30 text-blue-300 border border-blue-800/50 rounded-full">
              {kw}
              <button onClick={() => onChange({ meta_keywords: metaKeywords.filter(k => k !== kw) })}><X className="h-3 w-3" /></button>
            </span>
          ))}
        </div>
      </div>
