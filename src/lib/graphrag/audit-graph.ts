...
export type NodeType = 'commit' | 'deployment' | 'incident' | 'agent_vote' | 'fix' | 'transaction';
export type EdgeType = 'caused_by' | 'fixed_by' | 'voted_on' | 'triggered' | 'resolved' | 'approved_by';
...
export function traceReasoningChain(
  ...
  function traverse(nodeId: string, depth: number): void {
    ...
    const incomingEdges = graph.edges.filter(e =>
      e.to_id === nodeId && ['caused_by', 'triggered', 'voted_on'].includes(e.edge_type)
    );
...
export function createDeploymentNode(commitSha: string, status: 'success' | 'failure', metadata: Record<string, unknown>): GraphNode {
  return {
...
export function createAgentVoteNode(agentName: string, vote: 'approve' | 'reject', confidence: number, finding: string): GraphNode {
  return {
