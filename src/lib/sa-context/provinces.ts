...
export function rankByOpportunityUrgency(): ProvinceProfile[] {
  return [...SA_PROVINCES].sort(
    (a, b) =>
      b.unemploymentPercent +
      (100 - b.digitalAccessPercent) -
 ...
 */
export function buildProvinceContextSummary(
  province?: ProvinceProfile | null,
): string {
  if (!province) {
    ...
    return [
      ...
      ``,
      `KEY FACTS:`,
      SA_NATIONAL_CONTEXT.digitalEconomyHighlights
        .map((h) => `- ${h}`)
        .join("\n"),
      ``,
      `EDUCATION:`,
