    ...
    if (result.code === "40001" || result.error === "SERIALIZATION_FAILURE") {
      if (attempt >= MAX_RETRIES) {
        log({
          level: "warn",
          service: SERVICE,
          message: `Serialization failure after ${MAX_RETRIES} retries`,
          requestId,
        });
        return {
          success: false,
      ...
      log({
        level: "warn",
        service: SERVICE,
        message: `SQLSTATE 40001 — retry ${attempt + 1}/${MAX_RETRIES} in ${Math.round(delay)}ms`,
        requestId,
      });
