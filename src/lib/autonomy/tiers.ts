...
const HIGH_RISK_PATTERNS = [
  'treasury', 'withdrawal', 'transfer', 'private_key', 'secret',
  'admin', 'sudo', 'root', 'production', 'rollback',
];

const MEDIUM_RISK_PATTERNS = [
  'deploy', 'migration', 'schema', 'config', 'payment',
  'wallet', 'credential', 'permission', 'role',
];

export function classifyOperation(
  operation: string,
  context: { amountUsd?: number; isProduction?: boolean; affectsFinancials?: boolean } = {}
): OperationRisk {
  const op = operation.toLowerCase();

  // T4: Real money movements > $100 or key rotation
  if (context.amountUsd && context.amountUsd > 100) {
    return { tier: 'T4', riskScore: 0.9, requiresHITL: true, requiresByzantine: true,
      reason: `High-value financial operation ($${context.amountUsd})` };
  }

  // T3: Any financial operation, KYC, auth changes
  if (HIGH_RISK_PATTERNS.some(p => op.includes(p)) || context.affectsFinancials) {
    return { tier: 'T3', riskScore: 0.7, requiresHITL: true, requiresByzantine: true,
      reason: `High-risk operation: ${operation}` };
  }

  // T2: Deployments, migrations, config changes
  if (MEDIUM_RISK_PATTERNS.some(p => op.includes(p)) || context.isProduction) {
    return { tier: 'T2', riskScore: 0.4, requiresHITL: false, requiresByzantine: true,
      reason: `Medium-risk operation: ${operation}` };
  }

  // T1: Everything else — auto-execute
  return { tier: 'T1', riskScore: 0.1, requiresHITL: false, requiresByzantine: false,
...
export function assertAuthorized(
  operation: string,
  options: { humanApprovalToken?: string; context?: Parameters<typeof classifyOperation>[1] } = {}
): OperationRisk {
