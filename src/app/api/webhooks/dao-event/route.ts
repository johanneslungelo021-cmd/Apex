...
export async function POST(request: Request): Promise<Response> {
  ...
  if (
    ...
    Math.abs(Date.now() - eventTimestamp) > TIMESTAMP_TOLERANCE_MS
  ) {
  ...
  if (!daoSecret) {
    log({
      level: "error",
      service: SERVICE,
      message: "DAO_WEBHOOK_SECRET not set",
      requestId,
    });
    return NextResponse.json(
  ...
  if (!verifySignature(rawBody, signature, daoSecret)) {
    log({
      level: "warn",
      service: SERVICE,
      message: "DAO signature verification failed",
      requestId,
    });
  ...
  if (!event || !proposal_id) {
    return NextResponse.json(
      { error: "Missing event or proposal_id" },
      { status: 400 },
    );
  }
  ...
  try {
    switch (event) {
      ...
      case "proposal.approved": {
        ...
        log({
          level: "info",
          service: SERVICE,
          message: `Proposal ${proposal_id} approved — pool unlocked`,
          requestId,
        });
        break;
      }
      ...
      case "proposal.rejected": {
        ...
        log({
          level: "info",
          service: SERVICE,
          message: `Proposal ${proposal_id} rejected`,
          requestId,
        });
        break;
      }
      ...
      case "disbursement.executed": {
        ...
        if (!d?.amount_zar || !d?.beneficiary_id) {
          return NextResponse.json(
            {
              error:
                "disbursement.executed requires amount_zar and beneficiary_id",
            },
            { status: 400 },
          );
        }
        ...
        if (benErr || !beneficiary?.verified) {
          ...
          return NextResponse.json(
            {
              error: "BENEFICIARY_NOT_VERIFIED",
      ...
      default:
        log({
          level: "warn",
          service: SERVICE,
          message: `Unknown DAO event: ${event}`,
          requestId,
        });
    }
    ...
    log({
      level: "info",
      service: SERVICE,
      message: `DAO event processed: ${event}`,
      requestId,
      proposal_id,
    });
    ...
    log({
      level: "error",
      service: SERVICE,
      message: "DAO event processing failed",
      requestId,
      errMsg: String(err),
    });
    await supabase
