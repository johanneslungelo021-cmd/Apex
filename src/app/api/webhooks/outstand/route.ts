...
import {
  classifyEmotionState,
  applyEmotionMultiplier,
} from "@/lib/treasury/emotion-classifier";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (
    ...
    log({
      level: "warn",
      service: SERVICE,
      message: "Timestamp outside 5-minute window",
      requestId,
    });
  ...
  if (!outstandSecret) {
    log({
      level: "error",
      service: SERVICE,
      message: "OUTSTAND_WEBHOOK_SECRET not set",
      requestId,
    });
    return NextResponse.json(
  ...
  const { event, external_id, amount, currency, creator_id, post, metadata } =
    body;
  ...
  if (!event || !external_id || !amount || !creator_id) {
    return NextResponse.json(
      {
        error:
          "Missing required fields: event, external_id, amount, creator_id",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    let emotionState: "ecstatic" | "bullish" | "neutral" | "panicked" =
      "neutral";
