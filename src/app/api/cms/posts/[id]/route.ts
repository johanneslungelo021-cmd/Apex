...
async function getCreatorId(supabase: ReturnType<typeof getSupabaseClient>, userId: string): Promise<string | null> {
  ...
  if (error) throw new Error(`getCreatorId DB error: ${error.message}`);
  return data?.id ?? null;
}
...
export async function GET(req: Request, ctx: Ctx): Promise<Response> {
  ...
  try {
    ...
    if (showVersions) {
      // FIX: include content, excerpt, snapshot so editor rollback receives all data
...
export async function PATCH(req: Request, ctx: Ctx): Promise<Response> {
  ...
  try {
    ...
    // FIX: select * for complete snapshot — previously missing slug/SEO/scheduling fields
    ...
    // FIX: slug now destructured from body
    const { title, content, excerpt, cover_image_url, content_type, status, scheduled_at,
      slug, tags, seo_title, seo_description, meta_keywords, og_image_url, change_note } = body;

    const updates: Record<string, unknown> = {};
    if (title         !== undefined) updates.title         = title;
    if (slug          !== undefined) updates.slug          = slug;  // FIX: persist slug changes
    ...
    // FIX: optimistic concurrency — reject if another write raced ahead
    ...
    if (!post) {
      return NextResponse.json(
        { error: 'CONFLICT', message: 'Post was modified by another request. Please reload and retry.' },
        { status: 409 },
      );
    }

    // FIX: check version insert error — no silent failures
