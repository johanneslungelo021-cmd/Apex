  ...
  try {
    ...
    if (showVersions) {
      ...
      const { data: versions, error: versionsError } = await supabase
        ...
        .select(
          "id,version,title,content,excerpt,snapshot,change_note,changed_by,created_at",
        )
        .eq("post_id", id)
      ...
      if (versionsError) {
        log({
          level: "error",
          service: SERVICE,
          message: "Versions query failed",
          requestId,
          errMsg: versionsError.message,
        });
  ...
  try {
    ...
    if (!post) {
      return NextResponse.json(
        {
          error: "CONFLICT",
          message: "Post was modified concurrently. Please reload and retry.",
        },
        { status: 409 },
      );
    }
    ...
    if (insertError) {
      ...
      if (revertErr || !revertData) {
        log({
          ...
          requestId,
          message:
            "Compensating rollback did not match any row — post may have been updated concurrently",
        });
      }
      throw insertError;
    }
