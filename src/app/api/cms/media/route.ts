  ...
  try {
    ...
    if (!safeExt)
      return NextResponse.json(
        { error: "INVALID_TYPE", message: "File type not allowed" },
        { status: 400 },
      );
    if (file.size > MAX_SIZE)
      return NextResponse.json(
        { error: "FILE_TOO_LARGE", message: "Max 50 MB" },
        { status: 400 },
      );
    ...
    if (!uploadRes.ok) {
      ...
      log({
        ...
        storageError: errText,
      });
      return NextResponse.json(
        {
          error: "STORAGE_UPLOAD_FAILED",
          message:
            'File upload failed. Check that Supabase Storage bucket "cms-media" exists.',
        },
        { status: 502 },
      );
    }
    ...
    if (dbErr) {
      ...
      try {
        ...
        log({
          ...
          service: SERVICE,
          requestId,
          message: "Storage cleanup after DB failure also failed",
          errMsg: String(cleanupErr),
        });
      }
      throw dbErr;
    }
