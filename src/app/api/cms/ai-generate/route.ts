...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!Object.prototype.hasOwnProperty.call(SYSTEM_PROMPTS, type)) {
      return NextResponse.json({ error: 'INVALID_TYPE', message: `Unknown type: ${String(type)}` }, { status: 400 });
    }
    ...
    if (!groqKey) {
      ...
      log({ level: 'error', service: SERVICE, message: 'GROQ_API_KEY not set', requestId });
      return NextResponse.json({ error: 'SERVICE_MISCONFIGURED', message: 'AI generation not configured.' }, { status: 500 });
    }
    ...
    const userMessage = [
      ...
      `Tone: ${String(tone)}`,
      (type === 'content' || type === 'expand') ? `Length: ${String(length)}` : '',
    ].filter(Boolean).join('\n\n');
    ...
    try {
      ...
      return NextResponse.json(
        { error: isTimeout ? 'AI_TIMEOUT' : 'AI_UNAVAILABLE',
          message: isTimeout ? 'AI request timed out.' : 'AI temporarily unavailable.' },
        { status: 504 },
      );
    }
    clearTimeout(tid);
    ...
    if (!res.ok) {
      ...
      log({ level: 'warn', service: SERVICE, requestId,
        message: `Groq upstream error`, httpStatus: res.status,
