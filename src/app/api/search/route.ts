...
function classifyResult(title: string, url: string): SearchResult['category'] {
  ...
  if (/freelan|fiverr|upwork|earn|income|job|opportunit|hustl|gig/.test(t)) return 'opportunity';
  if (/jse|zar|rand|btc|crypto|xrp|market|index|rate|price/.test(t + u)) return 'market_data';
  if (/how to|guide|tutorial|learn|course|step.by.step/.test(t)) return 'guide';
  if (/news|report|update|breaking|latest|says|announces/.test(t)) return 'news';
  return 'general';
}
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (!rawQuery) return NextResponse.json({ error: 'Missing query parameter: q' }, { status: 400 });

  const query = rawQuery.replace(/<[^>]*>/g, '').replace(/[\x00-\x1F]/g, '').slice(0, 500);
  ...
  if (!apiKey) {
    log({ level: 'error', service: SERVICE, message: 'PERPLEXITY_API_KEY not configured', requestId });
  ...
  try {
    ...
    let parsed: { title?: string; source?: string; url?: string; snippet?: string; date?: string | null }[] = [];
    ...
    const body: SearchResponse = { results, query, cached: false, retrievedAt: new Date().toISOString() };
    cache.set(query, { data: body, ts: Date.now() });
    searchQueryCounter.add(1, { status: 'success' });
    log({ level: 'info', service: SERVICE, message: `Returned ${results.length} results`, requestId });
    return NextResponse.json(body);
  } catch (err) {
    clearTimeout(tid);
    const msg = err instanceof Error ? err.message : String(err);
    log({ level: 'error', service: SERVICE, message: `Search failed: ${msg}`, requestId });
    searchQueryCounter.add(1, { status: 'error' });
    return NextResponse.json({ error: 'Search failed', details: msg }, { status: 502 });
  }
}
