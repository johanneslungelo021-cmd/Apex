...
export async function GET(): Promise<Response> {
  const manifest = {
    ...
    version: "1.0.0",
    description:
      "Unified interface for AI skills: chat, web search, TTS, image generation, and VLM.",
    skills: {
      chat: {
        endpoint: "/api/skills",
        method: "POST",
  ...
  try {
    switch (action) {
      case "chat": {
        ...
        if (!Array.isArray(messages)) {
          return NextResponse.json(
            {
              error: "INVALID_INPUT",
              message: 'Chat requires a "messages" array.',
              requestId,
            },
            { status: 400, headers: { "X-Request-Id": requestId } },
          );
      ...
      case "search": {
        ...
        const numResults =
          typeof body.numResults === "number" ? body.numResults : 10;
      ...
      case "tts": {
        ...
        if (!text || typeof text !== "string") {
          return NextResponse.json(
            {
              error: "INVALID_INPUT",
              message: 'TTS requires a "text" string.',
              requestId,
            },
            { status: 400, headers: { "X-Request-Id": requestId } },
          );
      ...
      case "image": {
        ...
        if (!prompt || typeof prompt !== "string") {
          return NextResponse.json(
            {
              error: "INVALID_INPUT",
              message: 'Image generation requires a "prompt" string.',
              requestId,
            },
            { status: 400, headers: { "X-Request-Id": requestId } },
          );
      ...
      case "vlm": {
        ...
        if (!image || typeof image !== "string") {
          return NextResponse.json(
            {
              error: "INVALID_INPUT",
              message: 'VLM requires an "image" string (base64 or URL).',
              requestId,
            },
            { status: 400, headers: { "X-Request-Id": requestId } },
          );
    ...
    log({
      level: "error",
      service: SERVICE,
      message: `Skill error: ${internalMessage}`,
      requestId,
    });
