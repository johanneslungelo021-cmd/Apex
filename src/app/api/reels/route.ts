...
async function generateReelIdeas(requestId: string): Promise<ReelIdea[]> {
  ...
  log({
    level: "info",
    service: SERVICE,
    message: `Generated ${parsed.length} reel ideas`,
    requestId,
  });
...
export async function GET(req: Request): Promise<Response> {
  ...
  // Pillar 4: rate limit — 10 req/min per IP (video idea generation is expensive)
  const ip =
    req.headers.get("x-forwarded-for")?.split(",")[0].trim() ?? "unknown";
  const allowed = checkRateLimit(`reels:${ip}`, 10, 60_000);
    ...
    return NextResponse.json(
      {
        ...
        message: "Reel ideas unavailable right now.",
      },
      { status: 503 },
    );
  }
