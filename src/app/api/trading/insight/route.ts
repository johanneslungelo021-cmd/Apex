...
export async function POST(req: Request): Promise<Response> {
  ...
  const zarUsd = typeof (body as Record<string, unknown>)?.zarUsd === 'number'
    ? (body as Record<string, unknown>).zarUsd as number
    : null;
  const xrpZar = typeof (body as Record<string, unknown>)?.xrpZar === 'number'
  ...
  try {
    ...
    const insight: string = (raw?.choices?.[0]?.message?.content as string ?? '').trim();

    log({ level: 'info', service: SERVICE, message: 'Insight generated', requestId, volatile: isVolatile });

    return NextResponse.json({
      insight: insight || 'Market conditions nominal. Monitor ZAR corridor for opportunity.',
      state: isVolatile ? 'volatile' : 'stable',
      generatedAt: new Date().toISOString(),
    } satisfies InsightResponse);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    log({ level: 'error', service: SERVICE, message: 'Insight generation failed', requestId, error: msg });
    return NextResponse.json({ error: 'GENERATION_FAILED', message: msg }, { status: 503 });
  }
}
