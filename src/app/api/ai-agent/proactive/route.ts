...
async function preBuildTransaction(
  ...
  if (!xrplServiceUrl) {
    return {
      pre_signed: false,
      error:
        "XRPL service not configured. Set XRPL_SERVICE_URL environment variable.",
    };
  }
          ...
          try {
            ...
            if (preBuildResult.pre_signed) {
              ...
              send({
                type: "transaction_ready",
                intent,
                message: preBuildResult.error ?? "XRPL service unavailable.",
                timestamp: Date.now(),
              });
            }
          } catch (error) {
            send({
              type: "error",
              message: `Failed to pre-build: ${error instanceof Error ? error.message : String(error)}`,
              timestamp: Date.now(),
            });
          }
        }
