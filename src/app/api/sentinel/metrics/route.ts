  ...
  const handler = mppx.tempo.charge({ amount: MPP_PRICING.analyticsQuery })(
    async (_innerRequest: Request) => {
      ...
      const metrics = {
        ...
        // C — Cost
        cost: {
          total_platform_fees_zar: successTx.reduce(
            (s, t) => s + Number(t.platform_fee_zar),
            0,
          ),
          avg_fee_per_tx_zar:
            successTx.length > 0
              ? successTx.reduce((s, t) => s + Number(t.platform_fee_zar), 0) /
                successTx.length
              : 0,
          ...
          success_rate:
            transactions.length > 0
              ? ((successTx.length / transactions.length) * 100).toFixed(2) +
                "%"
              : "N/A",
          gateway_breakdown: transactions.reduce<Record<string, number>>(
            (acc, t) => {
