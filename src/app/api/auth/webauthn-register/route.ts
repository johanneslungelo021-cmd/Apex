...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    void Promise.resolve(
      supabase.rpc("purge_expired_webauthn_challenges"),
    ).catch(() => null);
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    registrationResponse =
      parsed.registrationResponse as RegistrationResponseJSON;
  } catch (err) {
    ...
    return NextResponse.json(
      {
        ...
        message: isZod
          ...
          : "registrationResponse is required.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (challengeRetrieveErr || !challengeRow) {
      if (challengeRetrieveErr) {
        log({
          ...
          service: SERVICE,
          message:
            "Challenge retrieval failed — DB error masked as 400 previously",
          requestId,
          userToken: hashForLog(session.userId),
    ...
    try {
      ...
      return NextResponse.json(
        { error: "ATTESTATION_FAILED", message: "Device verification failed." },
        { status: 403 },
      );
    }
    ...
    if (!verification.verified || !verification.registrationInfo) {
      return NextResponse.json(
        { error: "REGISTRATION_FAILED", message: "Registration not verified." },
        { status: 403 },
      );
    }
    ...
    if (insertError) {
      ...
      return NextResponse.json(
        { error: "DB_ERROR", message: "Failed to register passkey." },
        { status: 500 },
      );
    }
    ...
    if (deleteErr) {
      log({
        ...
        service: SERVICE,
        message:
          "Failed to delete used challenge — potential replay window in registration",
        requestId,
        userToken: hashForLog(session.userId),
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Passkey registered. Biometric login is now active.",
      },
      { status: 201 },
    );
  } catch (err) {
