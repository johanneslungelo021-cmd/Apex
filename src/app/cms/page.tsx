...
interface Post {
  ...
  cover_image_url: string | null; content_type: string; status: string;
  scheduled_at: string | null; published_at: string | null; tags: string[];
  word_count: number; read_time_mins: number; created_at: string; updated_at: string;
  version: number;
}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-zinc-400 mt-1 text-sm">Manage your content, drafts, and publications</p>
          </div>
          <Link href="/cms/new"
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-800">
                  <th className="text-left text-xs font-medium text-zinc-500 px-6 py-3 uppercase tracking-wide">Title</th>
                  <th className="text-left text-xs font-medium text-zinc-500 px-4 py-3 uppercase tracking-wide">Status</th>
                  <th className="text-left text-xs font-medium text-zinc-500 px-4 py-3 uppercase tracking-wide">Type</th>
                  <th className="text-left text-xs font-medium text-zinc-500 px-4 py-3 uppercase tracking-wide">Words</th>
                  <th className="text-left text-xs font-medium text-zinc-500 px-4 py-3 uppercase tracking-wide">Updated</th>
                  <th className="text-right text-xs font-medium text-zinc-500 px-6 py-3 uppercase tracking-wide">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(post => {
                  ...
                  return (
                    <tr key={post.id}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            {post.excerpt && (
                              <p className="text-xs text-zinc-500 mt-0.5 line-clamp-1">{post.excerpt}</p>
                            )}
                            {post.tags.length > 0 && (
                              <div className="flex gap-1 mt-1">
                                {post.tags.slice(0, 3).map(t => (
                                  <span key={t} className="text-xs px-1.5 py-0.5 bg-zinc-800 text-zinc-400 rounded">{t}</span>
                                ))}
                              </div>
                            )}
                          </div>
                      ...
                      <td className="px-4 py-4">
                        <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full ${s.bg} ${s.color}`}>
                          <StatusIcon className="h-3 w-3" />{s.label}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-xs text-zinc-400 capitalize">{post.content_type}</span>
                      </td>
                      <td className="px-4 py-4">
                        <div className="text-xs text-zinc-400">
                          <div>{post.word_count.toLocaleString()} words</div>
                          <div className="text-zinc-600">{post.read_time_mins} min read</div>
                        </div>
                      </td>
                      <td className="px-4 py-4">
