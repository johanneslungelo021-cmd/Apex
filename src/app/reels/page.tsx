...
function ReelCard({ idea }: { idea: ReelIdea }) {
  ...
  const colorClass =
    PLATFORM_COLORS[idea.platform] ?? "text-zinc-400 bg-zinc-500/10";
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-1.5">
            {idea.hashtags.map((tag) => (
              <span
                key={tag}
                className="text-xs text-zinc-500 glass px-2 py-0.5 rounded-lg"
              >
                #{tag}
              </span>
            ))}
...
export default function ReelsPage() {
  ...
  const filtered =
    platform === "All" ? ideas : ideas.filter((i) => i.platform === platform);
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-zinc-400 mt-2 max-w-xl">
              AI-generated video scripts and viral ideas for SA content
              creators. Ready to film.
            </p>
          </div>
          <button
