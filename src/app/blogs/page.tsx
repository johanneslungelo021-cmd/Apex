...
function BlogCard({ post }: { post: BlogPost }) {
  ...
  const colorClass =
    CATEGORY_COLORS[post.category] ?? "text-zinc-400 bg-zinc-500/10";
            ...
            <div className="mt-4 pt-4 border-t border-white/10">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {post.tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-1 text-xs text-zinc-500 glass px-2 py-1 rounded-lg"
                  >
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-zinc-400 mt-2 max-w-xl">
              AI-researched articles on SA digital income, updated every 30
              minutes via Perplexity Sonar.
            </p>
          </div>
          <button
          ...
          <div className="flex gap-2 flex-wrap mb-6">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() =>
                  startFilterTransition(() => setActiveFilter(cat))
                }
        ...
        <AnimatePresence mode="wait">
          ...
          {error && posts.length === 0 && (
            ...
            >
              ...
              <button
                onClick={() => void fetchPosts()}
                className="mt-4 glass px-4 py-2 rounded-xl text-sm hover:bg-white/15 transition"
              >
                Try again
              </button>
            </motion.div>
