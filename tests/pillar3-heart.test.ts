...
describe("Identity Matrix — buildApexIdentity()", () => {
  ...
  it("injects adaptive context when provided", () => {
    const identity = buildApexIdentity(
      "<adaptive_context>test context</adaptive_context>",
    );
...
describe("Code Switch — detectUserLanguageStyle()", () => {
  ...
  it('detects "eish" as isiZulu frustration marker', () => {
    const result = detectUserLanguageStyle(
      "Eish this transaction keeps failing",
    );
    expect(result.hasVernacular).toBe(true);
    expect(result.detectedLanguages).toContain("zu-ZA");
  ...
  it("returns no vernacular for formal English", () => {
    const result = detectUserLanguageStyle(
      "Kindly provide information regarding the blockchain mechanism.",
    );
...
describe("Code Switch — selectVernacularPhrase()", () => {
  it("selects isiZulu greeting for zu-ZA user", () => {
    const phrase = selectVernacularPhrase(
      "greeting_first",
      ["zu-ZA"],
      "casual",
    );
...
describe("Empathy Engine — humanizeError()", () => {
  ...
  it("handles NETWORK_TIMEOUT correctly", () => {
    ...
    expect(
      result.suggestedActions.some((a) => a.action === "retry_immediate"),
    ).toBe(true);
  });
  ...
  it("handles AI_GENERATION_FAILED with encouragement", () => {
    ...
    expect(
      result.suggestedActions.some((a) => a.action === "retry_generation"),
    ).toBe(true);
  });
...
describe("Empathy Engine — severityToColorClass()", () => {
  it("returns different classes for each severity level", () => {
    const classes = ["low", "medium", "high", "critical"].map((s) =>
      severityToColorClass(s as ApexError["severity"]),
    );
...
describe("Sentiment Analysis — analyzeSentimentLocal()", () => {
  it("detects frustration with English markers", () => {
    expect(analyzeSentimentLocal("This is broken and doesn't work!")).toBe(
      "frustrated",
    );
  });

  it('detects frustration with SA vernacular "eish"', () => {
    expect(analyzeSentimentLocal("Eish this is so frustrating")).toBe(
      "frustrated",
    );
  });

  it("detects frustration from all-caps message", () => {
    expect(analyzeSentimentLocal("THIS IS NOT WORKING AT ALL!!")).toBe(
      "frustrated",
    );
  });

  it('detects excitement with SA vernacular "sharp"', () => {
    expect(analyzeSentimentLocal("Sharp sharp! It finally worked!")).toBe(
      "excited",
    );
  });

  it('detects excitement with "yebo"', () => {
    expect(analyzeSentimentLocal("Yebo! Amazing! Got it done!")).toBe(
      "excited",
    );
  });

  it("detects confusion", () => {
    expect(analyzeSentimentLocal("I don't understand what this means??")).toBe(
      "confused",
    );
  });
  ...
  it("detects anxiety about funds safety", () => {
    expect(analyzeSentimentLocal("Is it safe? Will I lose my money?")).toBe(
      "anxious",
    );
  });
  ...
  it("returns neutral for calm messages", () => {
    expect(
      analyzeSentimentLocal("Please show me the transaction history"),
    ).toBe("neutral");
  });
...
describe("Identity Middleware — detectToneViolations()", () => {
  ...
  it("detects corporate jargon", () => {
    const violations = detectToneViolations(
      "We should leverage synergy to utilize resources.",
    );
  ...
  it("returns empty array for clean Apex voice", () => {
    const clean =
      `The XRP Ledger settles your transaction in 3-5 seconds — ` +
...
describe("Identity Middleware — validateTone()", () => {
  it("returns true for clean text", () => {
    expect(
      validateTone("Eish, I hear you — that transaction path ran dry."),
    ).toBe(true);
  });
...
describe("Identity Middleware — enrichMessagesSync()", () => {
  ...
  it("preserves original user messages", () => {
    ...
    expect(userMsgs.some((m) => m.content === "How does XRPL work?")).toBe(
      true,
    );
  });
...
describe("Integration — full empathy loop", () => {
  it("frustrated isiZulu user gets full enrichment pipeline", () => {
    const messages: ServerMessage[] = [
      {
        role: "user",
        content: "Eish my transaction keeps failing!! I tried 3 times!!",
      },
    ];
  ...
  it("excited new user gets appropriate welcome enrichment", () => {
    const messages: ServerMessage[] = [
      {
        role: "user",
        content: "I just got my first smart contract deployed! Sharp sharp!",
      },
    ];
    const enriched = enrichMessagesSync(messages, {
      userContext: { isFirstInteraction: true },
...
describe("Identity Middleware — useLocalSentiment routing (connectivity fix)", () => {
  ...
  it("useLocalSentiment:true detects excited user without HF API", async () => {
    const messages: ServerMessage[] = [
      {
        role: "user",
        content: "Sharp sharp! I finally deployed my first contract!",
      },
    ];
    const enriched = await enrichMessages(messages, {
      userContext: {},
  ...
  it("enrichMessagesSync produces identical system structure to enrichMessages(local)", async () => {
    ...
    const asyncResult = await enrichMessages(messages, {
      ...ctx,
      useLocalSentiment: true,
    });
