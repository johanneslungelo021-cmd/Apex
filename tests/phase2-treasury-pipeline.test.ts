...
describe("Outstand Webhook", () => {
  ...
  it("processes ZAR payment end-to-end and returns transaction_id", async () => {
    ...
    const req = makeOutstandRequest({
      event: "payment",
      external_id: "ext-zar-001",
      amount: 1000,
      currency: "ZAR",
      creator_id: "user-creator-1",
      post: {
        platform: "instagram",
        text: "Amazing milestone! 1000 subscribers!",
        likes: 450,
      },
    });
    const res = await POST(req);
  ...
  it("resolves customer_id from KYC when id_number provided", async () => {
    ...
    mockSupabaseChain.maybeSingle.mockImplementation(async () => {
      ...
      if (callCount === 2)
        return { data: { id: "customer-kyc-uuid" }, error: null }; // customers
    ...
    const req = makeOutstandRequest({
      event: "payment",
      external_id: "ext-kyc-001",
      amount: 500,
      currency: "ZAR",
      creator_id: "user-creator-3",
  ...
  it("does not block transaction when KYC lookup fails", async () => {
    ...
    const req = makeOutstandRequest({
      event: "payment",
      external_id: "ext-kyc-fail",
      amount: 300,
      currency: "ZAR",
      creator_id: "user-creator-4",
...
describe("Kimi Emotion Classifier", () => {
  ...
  const emotionCases: Array<
    [string, "ecstatic" | "bullish" | "neutral" | "panicked", number]
  > = [
    ["ecstatic", "ecstatic", 1.2],
    ["bullish", "bullish", 1.1],
    ["neutral", "neutral", 1.0],
  ...
  it.each(emotionCases)(
    ...
    async (state, expectedState, expectedMultiplier) => {
      ...
      const { classifyEmotionState } =
        await import("@/lib/treasury/emotion-classifier");
  ...
  it("returns neutral (1.00×) when Kimi API returns non-200", async () => {
    ...
    expect(result.fee_multiplier).toBe(1.0);
  ...
  it("applyEmotionMultiplier calculates correctly for all states", async () => {
    const { applyEmotionMultiplier } =
      await import("@/lib/treasury/emotion-classifier");
    expect(applyEmotionMultiplier(100, "ecstatic")).toBe(120.0);
    expect(applyEmotionMultiplier(100, "bullish")).toBe(110.0);
    expect(applyEmotionMultiplier(100, "neutral")).toBe(100.0);
    expect(applyEmotionMultiplier(100, "panicked")).toBe(85.0);
    // Rounding: 50 * 0.85 = 42.50
    expect(applyEmotionMultiplier(50, "panicked")).toBe(42.5);
  });
});
...
describe("FX Conversion", () => {
  ...
  it("converts USD using live rate when available", async () => {
    ...
    expect(result.amount_zar).toBe(1850.0);
    expect(result.rate_used).toBe(18.5);
    expect(result.rate_source).toBe("live");
  ...
  it("uses cache when available (skips live fetch)", async () => {
    ...
    expect(result.amount_zar).toBe(1005.0); // 50 * 20.10
...
describe("E2E: Outstand sad post → 0.85× fee → treasury", () => {
  ...
  it("ecstatic post receives 1.20× fee multiplier", async () => {
    ...
    expect(json.fee_multiplier).toBe(1.2);
    expect(mockRpc).toHaveBeenCalledWith(
      "insert_transaction_serializable",
