...
function getTokenFromRequest_inline(
...
describe("FIX #02 — Paystack HMAC SHA-512 signature verification (CRITICAL)", () => {
  ...
  it("rejects a signature computed with a wrong secret (forge attempt)", () => {
    const forgedSig = crypto
      .createHmac("sha512", "attacker_secret")
      .update(body)
      .digest("hex");
  ...
  it("rejects a valid signature when the body has been tampered", () => {
    ...
    expect(verifyPaystackSignature(tamperedBody, originalSig, secret)).toBe(
      false,
    );
  });
...
describe("FIX #03 — Error handling and Zod input validation (HIGH)", () => {
  ...
  it("internal error messages are never returned to clients (generic 500 contract)", () => {
    ...
    const internalError = new Error(
      "Supabase connection refused at 192.168.1.1:5432",
    );
...
describe("FIX #04 — PII-safe logging with SHA-256 hashing (HIGH)", () => {
  ...
  it("hashForLog is collision-resistant across distinct users", () => {
    expect(hashForLog("userA@apex.co.za")).not.toBe(
      hashForLog("userB@apex.co.za"),
    );
  });
...
describe("@vercel/firewall rate-limit — verify route contract", () => {
  ...
  it("@vercel/firewall exports checkRateLimit / unstable_checkRateLimit", async () => {
    ...
    expect(
      typeof fw.checkRateLimit === "function" ||
        typeof fw.unstable_checkRateLimit === "function",
    ).toBe(true);
  });
