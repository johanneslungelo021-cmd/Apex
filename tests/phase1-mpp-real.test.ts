...
const mockRpc = jest.fn().mockResolvedValue({
  data: { id: 'tx-mpp-001', status: 'success', gateway: 'tempo_mpp', mpp_intent: 'charge' },
  error: null,
});
...
describe('Tempo Chain Config', () => {
  ...
  it('USDC is the correct mainnet TIP-20 address (6 decimals)', async () => {
    ...
    expect(TIP20.USDC.toLowerCase()).toBe('0x20c000000000000000000000b9537d11c60e8b50');
  });

  it('pathUSD is the correct testnet TIP-20 address (6 decimals)', async () => {
    const { TIP20 } = await import('@/lib/payments/tempo-chain');
    expect(TIP20.pathUSD.toLowerCase()).toBe('0x20c0000000000000000000000000000000000000');
  });

  it('mainnet TempoStreamChannel escrow address is correct', async () => {
    const { TEMPO_CONTRACTS } = await import('@/lib/payments/tempo-chain');
    expect(TEMPO_CONTRACTS.mainnet.streamChannel.toLowerCase())
      .toBe('0x33b901018174ddabe4841042ab76ba85d4e24f25');
  });

  it('testnet TempoStreamChannel escrow address is correct', async () => {
    const { TEMPO_CONTRACTS } = await import('@/lib/payments/tempo-chain');
    expect(TEMPO_CONTRACTS.testnet.streamChannel.toLowerCase())
      .toBe('0xe1c4d3dce17bc111181ddf716f75bae49e61a336');
  });
...
describe('MPP Server Helpers', () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    jest.clearAllMocks();
    process.env = { ...OLD_ENV, APEX_TEMPO_RECIPIENT: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266' };
  });
  ...
  it('recordMppPayment is non-fatal when DB throws', async () => {
    ...
    const { recordMppPayment } = await import('@/lib/payments/mpp-server');
    await expect(recordMppPayment({
      creatorId: 'x', amountUsd: '0.001', mppIntent: 'charge', receiptReference: 'ref',
    }, 'req-test-003')).resolves.toBeUndefined();
  });
});
